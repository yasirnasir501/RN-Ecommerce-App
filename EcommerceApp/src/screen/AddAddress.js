...
const AddAddress = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput placeholder='Enter PinCode' placeholderTextColor={'#000'} keyboardType={'number-pad'} style={[styles.input, { marginTop: 15, color: '#000' }]} value={pincode} onChangeText={txt => setPinCode(txt)} />

      <View style={styles.typeView}>

        <TouchableOpacity style={[styles.typeBtn, { borderWidth: 0.5, borderColor: type == 1 ? 'orange' : 'black' }]} onPress={() => {
          setType(1);
        }}>
          <Image source={type == 1 ? require('../images/radioselect.png') : require('../images/radiounselect.png')} style={styles.radio} />
          <Text style={styles.radioText}>{'Home'}</Text>
        </TouchableOpacity>

        <TouchableOpacity style={[styles.typeBtn, { borderWidth: 0.3, borderColor: type == 2 ? 'orange' : 'black' }]} onPress={() => {
          setType(2)
        }}>
          <Image source={type == 2 ? require('../images/radioselect.png') : require('../images/radiounselect.png')} style={styles.radio} />
          <Text style={styles.radioText}>{'Office'}</Text>
        </TouchableOpacity>
      </View>

      <CustomButton bg={'#FE9000'} title={'Save Address'} color={'#fff'} onClick={() => {
